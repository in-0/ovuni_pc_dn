...
class OV_Uni3DETR(MVXTwoStageDetector):
    ...
    def forward_train(self,
        ...
        """
        # self.save_samples(points, img_metas, gt_bboxes_3d)
        # breakpoint()
    ...
    def save_samples(self, points, img_metas, gt_bboxes_3d):
        """
            img_metas: {'flip': False, 
                        ...
                        'pcd_trans': array([-0.08427241,  0.19699245,  0.12661185]), 
                        'sample_idx': 'scene0675_00', 
                        'pcd_scale_factor': 0.984730959867781, 
                        'pcd_rotation': tensor([[ 1.0000,  0.0078,  0.0000], [-0.0078,  1.0000,  0.0000], [ 0.0000,  0.0000,  1.0000]]), 
                        'pcd_rotation_angle': 0.007833551694814408, 
                        'pts_filename': 'data/scannet/points/scene0675_00.bin', 
                        'transformation_3d_flow': ['R', 'S', 'T']}
        """
        import mmcv
        import os
        # scene_idx = img_metas[0]['sample_idx']
        scene_idx = img_metas[0]['pts_filename'].split('/')[-1].split('.')[0]
        print(scene_idx)
        mmcv.mkdir_or_exist(self.sample_save_dir)
        points_path = os.path.join(self.sample_save_dir, img_metas[0]['pts_filename'].split('/')[-1])
        bboxes_path = os.path.join(self.sample_save_dir, scene_idx + '_bboxes.npy')
        points = points[0].cpu().numpy()
        gt_bboxes_3d = gt_bboxes_3d[0].tensor.cpu().numpy()
        points.tofile(points_path)
        np.save(bboxes_path, gt_bboxes_3d)
