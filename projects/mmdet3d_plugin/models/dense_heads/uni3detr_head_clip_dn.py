...
class Uni3DETRHeadCLIPDN(DETRHead):
    ...
    def __init__(self,
                 *args,
                 ...
                 num_cls_fcs=2,
                 code_weights=None,
                 train_zeroshot_path=None,
                 val_zeroshot_path=None,
                 multimodal=False,
        ...
        self.train_zeroshot_path = train_zeroshot_path
        train_zs_weights = np.load(self.train_zeroshot_path)
        train_zs_weights = torch.tensor(train_zs_weights, dtype=torch.float32)
        train_zs_weights = F.normalize(train_zs_weights, p=2,dim=1)
        train_zs_weights = train_zs_weights.permute(1, 0).contiguous()
        self.register_buffer('train_zs_weights', train_zs_weights)

        self.val_zeroshot_path = val_zeroshot_path
        val_zs_weights = np.load(self.val_zeroshot_path)
        val_zs_weights = torch.tensor(val_zs_weights, dtype=torch.float32)
        val_zs_weights = F.normalize(val_zs_weights, p=2,dim=1)
        val_zs_weights = val_zs_weights.permute(1, 0).contiguous()
        self.register_buffer('val_zs_weights', val_zs_weights)
