...
class Uni3DETRHeadCLIPDNTSRef(DETRHead):
    """Head of UVTR. 
    Args:
    ...
    def __init__(self,
        ...
        # zs_weights = self.load_zeroshot_weights(self.zeroshot_path)
    ...
    def load_zeroshot_weights(self, zeroshot_path):
        zs_weights = np.load(zeroshot_path)
        zs_weights = torch.tensor(zs_weights, dtype=torch.float32)
        zs_weights = F.normalize(zs_weights, p=2,dim=1)
        zs_weights = zs_weights.permute(1, 0).contiguous()
        return zs_weights
    ...
    def prepare_for_dn_cmt(self, batch_size, reference_points, img_metas, points=None):
        if self.training:
            ...
            batch_idx = torch.cat([torch.full((t.size(0), ), i) for i, t in enumerate(targets)]) # 각 gt가 어느 이미지에 속하는지
            ...
            known_bid = batch_idx.repeat(groups, 1).view(-1) # reference_points에 대한 batch index
    ...
    def distance(self, bbox_centers, ref_points):
        """
        Calculate the distance between the line passing through the origin and bbox_centers, and ref_points.
        Args:
            bbox_centers (Tensor): Bounding box centers, shape BS x (nq+padding) x 3
            ref_points: B x (nq+padding) x 3
        """
        # Normalize bbox_centers to get direction vectors
        line_directions = bbox_centers / torch.norm(bbox_centers, dim=-1, keepdim=True)

        # Calculate the vector from the origin to the ref_points
        vec_origin_to_ref = ref_points

        # Project vec_origin_to_ref onto line_directions
        proj_lengths = torch.sum(vec_origin_to_ref * line_directions, dim=-1, keepdim=True)
        proj_vecs = proj_lengths * line_directions
        
        # Calculate the perpendicular vector
        perp_vecs = vec_origin_to_ref - proj_vecs
        
        # Calculate the distance as the norm of the perpendicular vector
        distances = torch.norm(perp_vecs, dim=-1)
    ...
    def forward(self, pts_feats, img_metas, fpsbpts, gt_bboxes_3d=None, gt_bboxes=None, points=None):
        ...
        """
        tgt_embed = self.tgt_embed.weight           # nq * 2, 256
        ...
        hs, init_reference, inter_references, ts_reg_outs = self.transformer(
            pts_feats,
            query_embeds,
            self.num_query,
        ...
        if self.training:
            ...
            outs.update({
                ...
                'all_ts_reg_outs': ts_reg_outs[:, :, :-dn_pad_size, :],
                ...
                'dn_ts_reg_outs': ts_reg_outs[:, :, -dn_pad_size:, :],
    ...
    def dn_loss_single(self,
        ...
        ts_reg_outs = ts_reg_outs[(bid, map_known_indice)]
        ...
        # Transformer ref point loss
        bbox_centers = known_bboxs[:, :3]
        distances = self.distance(bbox_centers, ts_reg_outs)
        loss_ts_ref = torch.sum(distances)
        ...
        return loss_cls*self.dn_weight, loss_bbox*self.dn_weight, loss_iou*self.dn_weight, loss_iou_pred*self.dn_weight, loss_consistency*self.dn_weight, loss_ts_ref*self.dn_weight
    ...
    def loss(self,
        ...
        dn_ts_reg_outs = preds_dicts['dn_ts_reg_outs']
        ...
        # calculate class and box loss
        dn_losses_cls, dn_losses_bbox, dn_losses_iou, dn_losses_iou_pred, dn_losses_consistency, dn_losses_ref_pts = multi_apply(
            self.dn_loss_single, dn_cls_scores, dn_bbox_preds, dn_iou_preds, dn_uncertainty_preds, dn_ts_reg_outs, dn_mask_dict)
        ...
        loss_dict['dn_loss_ref_pts'] = dn_losses_ref_pts[-1]

        # loss from other decoder layers
        num_dec_layer = 0
        for loss_cls_i, loss_bbox_i, loss_iou_i, loss_iou_pred_i, loss_consistency_i, loss_ref_pts_i in zip(dn_losses_cls[:-1], dn_losses_bbox[:-1], dn_losses_iou[:-1], dn_losses_iou_pred[:-1], dn_losses_consistency[:-1], dn_losses_ref_pts[:-1]):
            ...
            loss_dict[f'd{num_dec_layer}.dn_loss_ref_pts'] = loss_ref_pts_i
            num_dec_layer += 1
