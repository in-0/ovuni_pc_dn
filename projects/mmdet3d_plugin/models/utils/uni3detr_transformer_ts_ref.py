...
class Uni3DETRTransformer(BaseModule):
    ...
    def forward(self,
        ...
        bs = pts_value.shape[0] # pts features
        reference_points = query_embed[..., self.d_model:] # reference points (bs, num_query, 3)
        query = query_embed[..., :self.d_model] # (bs, num_query, dim_embed)
        ...
        ts_reg_outs = []
        for g in range(ng):
            inter_states1, inter_references1, ts_reg_out = self.decoder(
            ...
            ts_reg_outs.append(ts_reg_out)

        inter_states = torch.cat(inter_states, 1)
        inter_references = torch.cat(inter_references, 2)
        ts_reg_outs = torch.cat(ts_reg_outs, 2)

        inter_references_out = inter_references.sigmoid()
        ts_reg_outs = ts_reg_outs.sigmoid()
        return inter_states, init_reference_out, inter_references_out, ts_reg_outs
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            if reg_branches is not None:
                ...
                tmp_copy = torch.cat((tmp[...,:2].clone(), tmp[...,4:5].clone()),dim=-1)
            ...
            if self.return_intermediate:
                ...
                ts_reg_outs.append(tmp_copy)

        if self.return_intermediate:
            return torch.stack(intermediate), torch.stack(intermediate_reference_points), torch.stack(ts_reg_outs)

        return output, reference_points, tmp_copy
